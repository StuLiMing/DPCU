...
from pcu.wandb_logger import WandbLogger
...
class TrainerBase:
    ...
    def setup_optimizaton(self):
        ...
        # amp settings
        self.amp_scaler = amp.GradScaler('cuda') if self.configs.train.use_amp else None
    ...
    def build_dataloader(self):
        ...
        # make datasets
        datasets = {'train':PCUDataSet(self.configs.data.train)}
        if hasattr(self.configs.data, 'val') and self.rank == 0:
            datasets['val'] = PCUDataSet(self.configs.data.val)
        if self.rank == 0:
    ...
    def train(self):
        self.use_wandb=self.configs.wandb.use_wandb
        self.num_gpus=torch.cuda.device_count()
        self.init_logger()       # setup logger: self.logger

        if self.use_wandb and ((self.num_gpus >1 and dist.get_rank() == 0) or self.num_gpus==1):
            self.wandb_logger=WandbLogger(self.configs)
        else:
...
class TrainerDifIR(TrainerBase):
    ...
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        ...
        with torch.amp.autocast(device_type="cuda"):
            ...
            losses['loss'] = losses['mae']
            loss = losses['loss'].mean() / num_grad_accumulate
            if self.use_wandb and ((self.num_gpus>1 and dist.get_rank()==0) or self.num_gpus==1):
                self.wandb_logger.log_metrics({"l1_loss":float(loss)})
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            micro_data = {}
            micro_data["gt"]=data["gt"][jj:jj+micro_batchsize,]
            micro_data["lq"]=data["lq"][jj:jj+micro_batchsize,]
            micro_data["sr"]=data["sr"][jj:jj+micro_batchsize,]
