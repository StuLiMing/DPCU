...
class PatchUnEmbed(nn.Module):
    ...
    Args:
        ...
        embed_dim (int): 输入的嵌入维度, 默认96.
        patch_size (tuple[int]): Patch的尺寸, 与PatchEmbed中保持一致.
        patch_norm (bool, optional): 是否使用归一化, 默认为 False.
    """

    def __init__(self, out_chans, embed_dim=96, patch_size=(1, 4), patch_norm=False):
        super().__init__()
        self.embed_dim = embed_dim
        self.patch_size = patch_size
        # 使用ConvTranspose2d完成上采样, 这里的kernel_size和stride均设置为patch_size
    ...
    def forward(self, x):
        '''
        Args:
            x: Tensor of shape (B, embed_dim, Ph, Pw)
        Returns:
            Tensor of shape (B, out_chans, H, W),
            其中 H = Ph * patch_size[0],  W = Pw * patch_size[1]
        '''
        x = self.proj(x)
        x = self.norm(x)
        return x
...
# Modified
# reverse op of window partition
...
class SwinTransformerBlock(nn.Module):
    ...
    """
    # Modified
    def __init__(self, dim, input_resolution, num_heads, window_size=(2,8), shift_size=0,
    ...
    def calculate_mask(self, x_size):
        """
        Generate attention mask for shifted window attention, supporting rectangular windows.
        
        Args:
            x_size: (H, W) — feature map height and width
        
        Returns:
            attn_mask: (nW, Wh*Ww, Wh*Ww)
        """
        H, W = x_size
        Wh, Ww = self.window_size
        Sh, Sw = self.shift_size

        img_mask = torch.zeros((1, 1, H, W))  # shape: (1, 1, H, W)
        ...
        mask_windows = window_partition(img_mask, (Wh, Ww))  # (nW, 1, Wh, Ww)
    ...
    def forward(self, x):
        ...
        x_windows = x_windows.view(-1, self.window_size[0] * self.window_size[1], C)  # (NW*B) x (Ws*Ws) x C
