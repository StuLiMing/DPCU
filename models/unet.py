...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.dims = dims
        if use_conv:
            self.conv = Conv_Range(self.channels,self.out_channels)
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        if use_conv:
            self.op = Conv_Range(self.channels,self.out_channels,stride=stride)
        else:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            nn.SiLU(),
            Conv_Range(channels,self.out_channels),
        )
        ...
        self.out_layers = nn.Sequential(
            ...
            zero_module(
                Conv_Range(self.out_channels,self.out_channels)
            ),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = Conv_Range(channels,self.out_channels)
        else:
...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        # x_{t-1} and condition(sr image)
        in_channels = in_channels*2
        
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(Conv_Range(in_channels,ch))]
        )
                ...
                if ds in attention_resolutions and jj==0:
                    layers.append(
                        BasicLayer(
                                ...
                                img_height=self.img_height*self.up_factor,
        ...
        self.out = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            Conv_Range(input_ch,out_channels)
        )

    def forward(self, x, timesteps, sr=None, mask=None):
        """
